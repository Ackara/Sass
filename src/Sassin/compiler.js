...
function compile(args) {
    ...
    sass.render(options, function (error, sassResult) {
        ...
        if (args.minify) {
            let minifiedResult = minify(cssFile, sassResult.css.toString(), args);
            let minifiedCss = minifiedResult.css.toString();
            let minifiedFile = args.getOutputPath();

            if (args.generateSourceMaps) {
                let finalMap = args.getSourceMapPath();
                minifiedCss = (minifiedCss + os.EOL + "/*# sourceMappingURL=" + path.relative(args.outputDirectory, finalMap) + " */");
                mergeSourceMap(minifiedResult.map.toString(), sassResult.map.toString(), path.relative(args.sourceMapDirectory, minifiedFile));
            }
...
function minify(cssFile, css, args) {
